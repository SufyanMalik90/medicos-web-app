...
const ConfirmModalPurchaser = ({ isOpen, onClose, purchaserId , setUpdate}) => {
  if (!isOpen) return null;
  

  async function handleDeleteInvoice() {
    try {
      const response = await api.delete("/api/delete-purchaser", {
        data: {
      ...
      if (response?.data?.success) {
        ...
        toast.error(response?.data?.message || "Failed to delete the Purchaser.");
      }
    } catch (error) {
      console.error("Error deleting Purchaser:", error);
      onClose();
      toast.error("Failed to delete the Purchaser.");
    }
  }
      ...
      <div className="relative w-full max-w-lg  rounded-lg bg-white p-4 shadow-lg">
        ...
        <div className="my-4 text-center">
          ...
          <h4 className="mt-4 text-lg font-semibold text-gray-800">
            Are you sure you want to delete this purchaser?
          </h4>
        </div>
...
export default ConfirmModalPurchaser;
