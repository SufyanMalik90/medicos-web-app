...
const ConfirmModalOrder = ({ isOpen, onClose, orderId , setUpdate}) => {
  ...
  async function handleDeleteInvoice() {
    try {
      const response = await api.delete("/api/delete-order", {
        data: {
          order_id: orderId,
        },
      });
      ...
      if (response?.data?.success) {
        ...
        toast.error(response?.data?.message || "Failed to delete the invoice.");
      }
    } catch (error) {
